...
// return 0, otherwise
int is_compile_needed(char *c_file, char *binary) {
    // Check binary exists, if not - return 1
    if (fopen(binary, "r") == NULL) {
        return 1;   
    }
    
    // Stat and compare times
    struct stat cFile;
    stat(c_file, &cFile);
    struct stat binaryFile;
    stat(binary, &binaryFile);
    // If C file modified more recent than binary - return 1
    if ((long)cFile.st_mtime > (long)binaryFile.st_mtime) {
        return 1;
    }
