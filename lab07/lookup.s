# Read 2 numbers and use them as indices into a 2d-array (42 x 24)
# x in $t0, y in $t1
# $s2 address of array (start)
# $s0 and $s1 used to calculate row and column indexes
# $t2 array index
...
array_indexing:
    mul     $s0, $t0, COLUMN_SIZE           
    mul     $s0, $s0, 4                     # $s0 has rowIndex 'x'
    mul     $s1, $t1, 4                     # $s1 has colIndex 'y'
    
    add     $t2, $s2, $s0                   # $t2 = array[x]
    add     $t2, $t2, $s1                   # $t2 = array[x][y]
